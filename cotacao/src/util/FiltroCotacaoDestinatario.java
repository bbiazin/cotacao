...
public class FiltroCotacaoDestinatario {
	...
	public TypedQuery<ItemCotacao> getTypedQuery(EntityManager em) {
		...
		if (dono != null) {
			condicoes.add(new Object[] {"dono", "x.dono = ", dono});
		}
		if (habilitado != null) {
			condicoes.add(new Object[] {"email", "h.contato.email = ", habilitado.getEmail()});
			System.out.println(habilitado.getEmail());
		}

		String sql = "SELECT DISTINCT x FROM ItemCotacao x JOIN x.cotacao.habilitacoes h";
		if (condicoes.size() > 0) {
			sql += " WHERE";
		}
	...
	public Conta getHabilitado() {
		return habilitado;
	}

	public void setHabilitado(Conta habilitado) {
